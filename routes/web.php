...
Route::middleware(['setData'])->group(function () {
    ...
    // `laravel/ui` package (which adds this helper) may not be installed and
    // Auth::routes() will throw a RuntimeException.
    //
    // If the standard auth controllers generated by Laravel exist in
    // App\Http\Controllers\Auth, register the common auth routes manually
    // so the app works even when `laravel/ui` is not present.
    if (class_exists(\App\Http\Controllers\Auth\LoginController::class)) {
        ...
        // Registration (only if controller exists)
        if (class_exists(\App\Http\Controllers\Auth\RegisterController::class)) {
            Route::get('register', [App\Http\Controllers\Auth\RegisterController::class, 'showRegistrationForm'])->name('register');
            Route::post('register', [App\Http\Controllers\Auth\RegisterController::class, 'register']);
        }

        // Email verification (if controller exists)
        if (class_exists(\App\Http\Controllers\Auth\VerificationController::class)) {
            Route::get('email/verify', [App\Http\Controllers\Auth\VerificationController::class, 'show'])->name('verification.notice');
            Route::get('email/verify/{id}/{hash}', [App\Http\Controllers\Auth\VerificationController::class, '__invoke'])->name('verification.verify');
